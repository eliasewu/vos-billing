...
import { BarChart3, RefreshCw, Search, Download, Calendar, X } from "lucide-react";

interface CallAnalysis { prefix: string; totalCalls: number; successCalls: number; asr: number; totalDuration: number; acd: number; }

interface Props { gatewayType: "Mapping" | "Routing"; }

export default function CallAnalysisView({ gatewayType }: Props) {
  const [data, setData] = useState<CallAnalysis[]>([]);
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (d.error) { setError(d.error); setData([]); }
        ...
        const prefixMap = new Map<string, { calls: number; success: number; duration: number }>();
        for (const cdr of cdrs) {
          const prefix = (cdr.calleeE164 || "").slice(0, 6) || "Unknown";
          const existing = prefixMap.get(prefix) || { calls: 0, success: 0, duration: 0 };
          existing.calls++;
          if (cdr.endReason === 0) existing.success++;
          existing.duration += (cdr.feeTime || 0);
          prefixMap.set(prefix, existing);
        }
        const analysis: CallAnalysis[] = Array.from(prefixMap.entries())
          .map(([prefix, v]) => ({
            prefix, totalCalls: v.calls, successCalls: v.success,
            asr: v.calls > 0 ? Number(((v.success / v.calls) * 100).toFixed(1)) : 0,
            totalDuration: v.duration, acd: v.success > 0 ? Math.round(v.duration / v.success) : 0,
          }))
  ...
  const filtered = data.filter(d => d.prefix.includes(search));
  const totalCalls = data.reduce((s, d) => s + d.totalCalls, 0);
  const totalSuccess = data.reduce((s, d) => s + d.successCalls, 0);

  const exportCSV = () => {
    const h = ["Prefix", "TotalCalls", "SuccessCalls", "ASR%", "Duration", "ACD(s)"];
    const csv = [h.join(","), ...filtered.map(d => [d.prefix, d.totalCalls, d.successCalls, d.asr, d.totalDuration, d.acd].join(","))].join("\n");
    const b = new Blob([csv], { type: "text/csv" });
    const a = document.createElement("a"); a.href = URL.createObjectURL(b);
    a.download = `${gatewayType.toLowerCase()}_call_analysis.csv`; a.click();
  };
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div><h1 className="text-2xl font-bold text-surface-50">{gType} — Call Analysis</h1><p className="text-surface-400 text-sm mt-1">Call distribution by destination prefix{gType === "Routing" ? " for routing gateways" : ""}</p></div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="bg-surface-900 border border-surface-700/50 rounded-xl p-5"><p className="text-xs text-surface-500 mb-1">Success Calls</p><p className="text-2xl font-bold text-emerald-400">{totalSuccess.toLocaleString()}</p></div>
        ...
        <div className="bg-surface-900 border border-surface-700/50 rounded-xl p-5"><p className="text-xs text-surface-500 mb-1">Prefixes</p><p className="text-2xl font-bold text-violet-400">{data.length}</p></div>
      </div>

      <div className="relative max-w-md"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-surface-500" /><input type="text" placeholder="Search by prefix..." value={search} onChange={e => setSearch(e.target.value)} className="w-full pl-10 pr-4 py-2.5 bg-surface-900 border border-surface-700/50 rounded-lg text-surface-50 text-sm placeholder:text-surface-600 focus:outline-none focus:border-brand-500/50" /></div>
      ...
      <div className="bg-surface-900 border border-surface-700/50 rounded-xl overflow-hidden">
        <div className="overflow-x-auto"><table className="w-full text-sm">
          ...
          <tbody>{loading ? <tr><td colSpan={6} className="p-6 text-center"><RefreshCw className="w-6 h-6 animate-spin mx-auto text-surface-500" /></td></tr> :
            filtered.length === 0 ? <tr><td colSpan={6} className="p-12 text-center text-surface-500"><BarChart3 className="w-10 h-10 mx-auto mb-2 text-surface-600" /><p>No call data — adjust date range</p></td></tr> :
            filtered.map((d, i) => (
              <tr key={d.prefix + i} className="border-b border-surface-800/50 hover:bg-surface-800/30">
                <td className="px-4 py-3 text-surface-50 font-mono text-xs">{d.prefix}</td>
                ...
                <td className="px-4 py-3 text-right text-xs"><span className={`inline-flex px-2 py-0.5 rounded font-mono ${d.asr > 50 ? "bg-emerald-500/10 text-emerald-400" : d.asr > 0 ? "bg-amber-500/10 text-amber-400" : "bg-surface-800 text-surface-500"}`}>{d.asr}%</span></td>
