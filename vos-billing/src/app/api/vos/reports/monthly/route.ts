...
export async function GET() {
  ...
  try {
    const partitions = cdrPartitionsForLastNDays(90); // 3 months for monthly view
    const tables = await findExistingCdrPartitions(queryVos, partitions);

    // Aggregate across partitions in memory, grouped by month
