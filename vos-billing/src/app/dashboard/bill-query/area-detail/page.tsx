...
import { MapPin, Search, RefreshCw, Download, Calendar, X } from "lucide-react";

interface AreaBill { areaCode: string; totalCalls: number; successCalls: number; totalFee: number; totalDuration: number; asr: number; acd: number; }

export default function AreaDetailPage() {
  const [data, setData] = useState<AreaBill[]>([]);
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (d.error) { setError(d.error); setData([]); }
        ...
        const areaMap = new Map<string, { calls: number; success: number; duration: number; fee: number }>();
        for (const cdr of cdrs) {
          const area = cdr.calleeAreaCode || cdr.callerAreaCode || "Unknown";
          const existing = areaMap.get(area) || { calls: 0, success: 0, duration: 0, fee: 0 };
          existing.calls++;
        ...
        setData(Array.from(areaMap.entries()).map(([area, v]) => ({
          areaCode: area, totalCalls: v.calls, successCalls: v.success, totalFee: v.fee, totalDuration: v.duration,
  ...
  const filtered = data.filter(d => d.areaCode.toLowerCase().includes(search.toLowerCase()));
  const totalFee = data.reduce((s, d) => s + d.totalFee, 0);
  const totalCalls = data.reduce((s, d) => s + d.totalCalls, 0);

  const exportCSV = () => {
    const csv = ["AreaCode,Calls,Success,ASR%,ACD,Duration,Revenue", ...filtered.map(d => [d.areaCode, d.totalCalls, d.successCalls, d.asr, d.acd, d.totalDuration, Number(d.totalFee).toFixed(4)].join(","))].join("\n");
    const b = new Blob([csv], { type: "text/csv" }); const a = document.createElement("a"); a.href = URL.createObjectURL(b); a.download = "area_details.csv"; a.click();
  };

  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div><h1 className="text-2xl font-bold text-surface-50">Area Detail</h1><p className="text-surface-400 text-sm mt-1">Bill and revenue breakdown per area code</p></div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-surface-900 border border-surface-700/50 rounded-xl p-5"><p className="text-xs text-surface-500 mb-1">Area Codes</p><p className="text-2xl font-bold text-surface-50">{data.length}</p></div>
        ...
        <div className="bg-surface-900 border border-surface-700/50 rounded-xl p-5"><p className="text-xs text-surface-500 mb-1">Avg/Area</p><p className="text-2xl font-bold text-amber-400">{data.length > 0 ? formatMoney(totalFee / data.length) : "$0"}</p></div>
      </div>

      <div className="relative max-w-md"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-surface-500" /><input type="text" placeholder="Search by area code..." value={search} onChange={e => setSearch(e.target.value)} className="w-full pl-10 pr-4 py-2.5 bg-surface-900 border border-surface-700/50 rounded-lg text-surface-50 text-sm placeholder:text-surface-600 focus:outline-none focus:border-brand-500/50" /></div>
      ...
      <div className="bg-surface-900 border border-surface-700/50 rounded-xl overflow-hidden">
        <div className="overflow-x-auto"><table className="w-full text-sm">
          ...
          <tbody>{loading ? <tr><td colSpan={7} className="p-6 text-center"><RefreshCw className="w-6 h-6 animate-spin mx-auto text-surface-500" /></td></tr> :
            filtered.length === 0 ? <tr><td colSpan={7} className="p-12 text-center text-surface-500"><MapPin className="w-10 h-10 mx-auto mb-2 text-surface-600" /><p>No area detail data — adjust date range</p></td></tr> :
            filtered.map((d, i) => (
