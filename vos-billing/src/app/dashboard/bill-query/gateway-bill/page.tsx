...
import { Search, RefreshCw, Download, Calendar, X, Server } from "lucide-react";

interface GatewayBill { gatewayId: string; gatewayName: string; totalCalls: number; successCalls: number; totalDuration: number; totalFee: number; asr: number; acd: number; }

export default function GatewayBillPage() {
  const [data, setData] = useState<GatewayBill[]>([]);
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (d.error) { setError(d.error); setData([]); }
        ...
        const gwMap = new Map<string, { calls: number; success: number; duration: number; fee: number }>();
        for (const cdr of cdrs) {
          const gwId = cdr.callerGatewayId || cdr.calleeGatewayId || "Unknown";
          const existing = gwMap.get(gwId) || { calls: 0, success: 0, duration: 0, fee: 0 };
          existing.calls++;
          if (cdr.endReason === 0) existing.success++;
          existing.duration += (cdr.feeTime || 0);
          existing.fee += (cdr.fee || 0);
          gwMap.set(gwId, existing);
        }
        setData(Array.from(gwMap.entries()).map(([gwId, v]) => ({
          gatewayId: gwId, gatewayName: gwId,
          totalCalls: v.calls, successCalls: v.success, totalDuration: v.duration, totalFee: v.fee,
          asr: v.calls > 0 ? Number(((v.success / v.calls) * 100).toFixed(1)) : 0,
          acd: v.success > 0 ? Math.round(v.duration / v.success) : 0,
        })).sort((a, b) => b.totalCalls - a.totalCalls));
      }
    } catch { setError("Failed to load gateway bills"); }
  ...
  const filtered = data.filter(d => d.gatewayName.toLowerCase().includes(search.toLowerCase()) || d.gatewayId.toLowerCase().includes(search.toLowerCase()));
  const totalFee = data.reduce((s, d) => s + d.totalFee, 0);
  const totalCalls = data.reduce((s, d) => s + d.totalCalls, 0);

  const exportCSV = () => {
    const csv = ["Gateway,Calls,Success,ASR%,ACD,Duration,Revenue", ...filtered.map(d => [d.gatewayName, d.totalCalls, d.successCalls, d.asr, d.acd, d.totalDuration, Number(d.totalFee).toFixed(4)].join(","))].join("\n");
    const b = new Blob([csv], { type: "text/csv" }); const a = document.createElement("a"); a.href = URL.createObjectURL(b); a.download = "gateway_bills.csv"; a.click();
  };

  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div><h1 className="text-2xl font-bold text-surface-50">Gateway Bill</h1><p className="text-surface-400 text-sm mt-1">Bill and revenue breakdown per gateway</p></div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-surface-900 border border-surface-700/50 rounded-xl p-5"><p className="text-xs text-surface-500 mb-1">Gateways</p><p className="text-2xl font-bold text-surface-50">{data.length}</p></div>
        ...
        <div className="bg-surface-900 border border-surface-700/50 rounded-xl p-5"><p className="text-xs text-surface-500 mb-1">Avg/Gateway</p><p className="text-2xl font-bold text-amber-400">{data.length > 0 ? formatMoney(totalFee / data.length) : "$0"}</p></div>
      </div>

      <div className="relative max-w-md"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-surface-500" /><input type="text" placeholder="Search by gateway..." value={search} onChange={e => setSearch(e.target.value)} className="w-full pl-10 pr-4 py-2.5 bg-surface-900 border border-surface-700/50 rounded-lg text-surface-50 text-sm placeholder:text-surface-600 focus:outline-none focus:border-brand-500/50" /></div>
      ...
      <div className="bg-surface-900 border border-surface-700/50 rounded-xl overflow-hidden">
        <div className="overflow-x-auto"><table className="w-full text-sm">
          <thead><tr className="border-b border-surface-800">
            <th className="text-left px-4 py-3 text-surface-400 text-xs uppercase">Gateway ID</th>
          ...
          <tbody>{loading ? <tr><td colSpan={7} className="p-6 text-center"><RefreshCw className="w-6 h-6 animate-spin mx-auto text-surface-500" /></td></tr> :
            filtered.length === 0 ? <tr><td colSpan={7} className="p-12 text-center text-surface-500"><Server className="w-10 h-10 mx-auto mb-2 text-surface-600" /><p>No gateway bill data — adjust date range</p></td></tr> :
            filtered.map((d, i) => (
              <tr key={d.gatewayId + i} className="border-b border-surface-800/50 hover:bg-surface-800/30">
                <td className="px-4 py-3 text-surface-50 font-mono text-xs">{d.gatewayId}</td>
