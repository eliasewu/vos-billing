...
import { Clock, Package, RefreshCw, Trash2, Download, Upload, Plus, Edit2, X, Search } from "lucide-react";
...
export default function Page() {
  ...
  const fetchData = async () => { setLoading(true); setError(""); try { const r=await fetch("/api/vos/packages/free-duration"); const d=await r.json(); if(d.error)setError(d.error); else setProducts(d.products||[]); } catch { setError("Failed"); } finally { setLoading(false); }; };
  useEffect(()=>{fetchData();},[]);

  const handleDelete = async (id:number) => { if(!confirm("Delete this free duration entry?"))return; try { await fetch("/api/vos/packages/free-duration?id="+id,{method:"DELETE"}); setSuccess("Entry deleted"); fetchData(); } catch { setError("Failed to delete"); } };
  const handleSave = async () => { if(!form.packageId)return; setSaving(true); setError(""); try { const url = editingProduct ? `/api/vos/packages/free-duration/${editingProduct!.id}` : "/api/vos/packages/free-duration"; const method = editingProduct ? "PUT" : "POST"; const r = await fetch(url,{method,headers:{"Content-Type":"application/json"},body:JSON.stringify(form)}); const d = await r.json(); if(d.error) setError(d.error); else { setSuccess(editingProduct ? "Updated" : "Created"); setShowModal(false); setEditingProduct(null); fetchData(); } } catch { setError("Failed"); } finally { setSaving(false); } };
  ...
  const handleImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (const line of lines.slice(1)) {
      ...
      try {
        const r = await fetch("/api/vos/packages/free-duration", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ packageId: parseInt(row.packageid)||0, productType: row.producttype||"", freeDuration: parseInt(row.freeduration)||0, periodRate: parseFloat(row.periodrate)||0 }) });
  ...
  return (<div className="p-6 space-y-6">
    <div className="flex items-center justify-between"><div><h1 className="text-2xl font-bold text-surface-50">Package Free Duration</h1><p className="text-surface-400 text-sm mt-1">{products.length} entries | Free call duration per package</p></div>
    <div className="flex items-center gap-2">
      ...
      <button onClick={openAdd} className="flex items-center gap-1.5 px-4 py-2 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white text-sm font-medium"><Plus className="w-4 h-4"/>Add Duration</button>
    ...
    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
      <div className="bg-surface-900 border border-surface-700/50 rounded-xl p-5"><div className="flex items-center gap-3"><div className="w-10 h-10 rounded-lg bg-brand-500/10 flex items-center justify-center"><Package className="w-5 h-5 text-brand-400"/></div><div><p className="text-2xl font-bold text-surface-50">{products.length}</p><p className="text-xs text-surface-400">Total Entries</p></div></div></div>
      <div className="bg-surface-900 border border-surface-700/50 rounded-xl p-5"><div className="flex items-center gap-3"><div className="w-10 h-10 rounded-lg bg-emerald-500/10 flex items-center justify-center"><Clock className="w-5 h-5 text-emerald-400"/></div><div><p className="text-2xl font-bold text-surface-50">{formatSec(products.reduce((s,p)=>s+(p.freeDuration||0),0))}</p><p className="text-xs text-surface-400">Total Free Duration</p></div></div></div>
